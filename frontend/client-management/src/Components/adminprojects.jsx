...
const AdminProjectsContent = () => {
  ...
  const filteredProjects = projects.filter(project => {
    ...
    if (activeFilter === 'Overdue') return project.status === 'Overdue'
    return true
  })
            ...
            <table className="w-full">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  ...
                  <th className="px-6 py-4 text-left text-sm font-semibold text-slate-700">Team Size</th>
              ...
              <tbody className="divide-y divide-slate-200">
                {filteredProjects.map((project) => (
                  <tr key={project.id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-6 py-4 text-sm text-slate-800 font-medium">{project.name}</td>
                    <td className="px-6 py-4 text-sm text-slate-800">{project.client}</td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-sm text-slate-800">{project.progress}%</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-sm text-slate-800">{project.team} members</td>
                    <td className="px-6 py-4">
                      <span className={`px-3 py-1 rounded-full text-xs font-medium ${getPriorityColor(project.priority)}`}>
                        {project.priority}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-sm text-slate-800">{project.dueDate}</td>
